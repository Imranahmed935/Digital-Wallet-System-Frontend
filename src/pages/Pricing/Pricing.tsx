import {
  ArrowDownCircle,
  ArrowUpCircle,
  Send,
  CreditCard,
} from "lucide-react";

const Pricing = () => {
  const pricingOptions = [
    {
      title: "Cash In",
      description: "Add money to your wallet instantly from banks or agents.",
      fee: "Free",
      icon: <ArrowDownCircle className="w-12 h-12 text-violet-600" />,
    },
    {
      title: "Cash Out",
      description: "Withdraw money easily from agents or ATMs.",
      fee: "1.5% per transaction",
      icon: <ArrowUpCircle className="w-12 h-12 text-violet-600" />,
    },
    {
      title: "Send Money",
      description: "Transfer money to friends and family securely.",
      fee: "৳5 flat fee",
      icon: <Send className="w-12 h-12 text-violet-600" />,
    },
    {
      title: "Bill Payments",
      description: "Pay utility bills, internet, and more from your wallet.",
      fee: "৳10 per bill",
      icon: <CreditCard className="w-12 h-12 text-violet-600" />,
    },
  ];

  return (
    <section
      id="pricing-section"
      className="relative md:py-32 py-20 bg-gray-50 dark:bg-gray-900"
    >
      <div className="container mx-auto max-w-6xl px-6">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-extrabold text-gray-900 dark:text-white">
            <span className="text-violet-600">ZPay's</span> Transaction Fees
          </h2>
          <p className="mt-4 text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">
            Simple, transparent, and affordable fees for every transaction you make.
          </p>
        </div>
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
          {pricingOptions.map((option, idx) => (
            <div
              key={idx}
              className="bg-white dark:bg-gray-800 rounded-3xl shadow-lg p-8 text-center border border-transparent 
                         hover:border-violet-500 hover:shadow-2xl transition-all duration-300 ease-in-out group"
            >
              <div className="flex justify-center mb-6 transform transition-transform duration-500 group-hover:scale-110">
                {option.icon}
              </div>
              <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">
                {option.title}
              </h3>
              <p className="text-gray-600 dark:text-gray-300 text-sm mb-4">
                {option.description}
              </p>
              <p className="text-lg font-bold text-violet-600">{option.fee}</p>
              <button className="mt-6 px-6 py-2 bg-violet-600 text-white rounded-full font-medium hover:bg-violet-700 transition">
                Choose
              </button>
            </div>
          ))}
        </div>
      </div>
     
    
    </section>
  );
};

export default Pricing;
